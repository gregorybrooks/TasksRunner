...
if [ "$BETTER_PATH" = "ABSTRACT-B-1" ] || [ "$BETTER_PATH" = "ABSTRACT-B-2" ] ||
    ...
    if [ "$START_STAGE" = "translate-en-fa" ] || [ $STARTED = "true" ]; then
        ...
        STARTED="true"
        time docker run --env CURRENT_STAGE=$CURRENT_STAGE $ADDITIONAL_FLAGS \
             --gpus 1 --entrypoint $ISI_HOME/scripts/translate-mbart.sh \
    ...
    if [ "$START_STAGE" = "postprocess-train-data" ] || [ $STARTED = "true" ]; then
        ...
        STARTED="true"
        time docker run --env CURRENT_STAGE=$CURRENT_STAGE $ADDITIONAL_FLAGS \
             --gpus 1 --entrypoint $ISI_HOME/scripts/postprocess-projections.sh \
    ...
    if [ "$START_STAGE" = "detect-triggers" ] || [ $STARTED = "true" ]; then
        ...
        STARTED="true"
        time docker run --env CURRENT_STAGE=$CURRENT_STAGE $ADDITIONAL_FLAGS \
             --gpus 1 --entrypoint $ISI_HOME/scripts/detect-abstract-farsi-triggers.sh \
    ...
    if [ "$START_STAGE" = "attach-arguments" ] || [ $STARTED = "true" ]; then
        ...
        STARTED="true"
        time docker run --env CURRENT_STAGE=$CURRENT_STAGE $ADDITIONAL_FLAGS \
             --gpus 1 --entrypoint $ISI_HOME/scripts/abstract-attach-arguments.sh \
    ...
    if [ "$START_STAGE" = "postprocess" ] || [ $STARTED = "true" ]; then
        ...
        STARTED="true"
        time docker run --env CURRENT_STAGE=$CURRENT_STAGE $ADDITIONAL_FLAGS \
             --gpus 1 --entrypoint $ISI_HOME/scripts/postprocess.sh \
             ${DOCKER_IMAGE_PREFIX}-event-extractor-boston:$DOCKER_IMAGE_TAG
        if [ "$END_STAGE" = "$CURRENT_STAGE" ]; then exit; fi
    fi
elif [ "$BETTER_PATH" = "BASIC-B-1" ]; then
    if [ "$START_STAGE" = "ingest" ]; then
        echo "---------------- 1. INGEST ----------------"
        CURRENT_STAGE="ingest"
        STARTED="true"
    ...
    if [ "$START_STAGE" = "detect-triggers" ] || [ $STARTED = "true" ]; then
        ...
        STARTED="true"
        time docker run --env CURRENT_STAGE=$CURRENT_STAGE $ADDITIONAL_FLAGS \
             --gpus 1 --entrypoint $ISI_HOME/scripts/detect-basic-farsi-triggers.sh \
             ${DOCKER_IMAGE_PREFIX}-bert-bio:$DOCKER_IMAGE_TAG
        if [ "$END_STAGE" = "$CURRENT_STAGE" ]; then exit; fi
    fi
    ...
    if [ "$START_STAGE" = "extract-event-event-relations" ] || [ $STARTED = "true" ]; then
        ...
        STARTED="true"
        time docker run --env CURRENT_STAGE=$CURRENT_STAGE $ADDITIONAL_FLAGS \
             --gpus 1 --entrypoint $ISI_HOME/scripts/extract-event-event-relations.sh \
             ${DOCKER_IMAGE_PREFIX}-argument-attachment:$DOCKER_IMAGE_TAG
        if [ "$END_STAGE" = "$CURRENT_STAGE" ]; then exit; fi
    fi
    ...
    if [ "$START_STAGE" = "attach-arguments" ] || [ $STARTED = "true" ]; then
        ...
        STARTED="true"
        time docker run --env CURRENT_STAGE=$CURRENT_STAGE $ADDITIONAL_FLAGS \
             --gpus 1 --entrypoint $ISI_HOME/scripts/basic-attach-arguments.sh \
             ${DOCKER_IMAGE_PREFIX}-argument-attachment:$DOCKER_IMAGE_TAG
        if [ "$END_STAGE" = "$CURRENT_STAGE" ]; then exit; fi
    fi
...
elif [ "$BETTER_PATH" = "BASIC-B-2" ]; then
    if [ "$START_STAGE" = "ingest" ]; then
        echo "---------------- 1. INGEST ----------------"
        CURRENT_STAGE="ingest"
        STARTED="true"
...
elif [ "$BETTER_PATH" = "BASIC-E-1" ]; then
    if [ "$START_STAGE" = "ingest" ]; then
        echo "---------------- 1. INGEST ----------------"
        CURRENT_STAGE="ingest"
        STARTED="true"
